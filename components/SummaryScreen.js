...
const SummaryScreen = ({ history, totalTime, onRestart }) => {
  const totalAttempts = history.reduce((acc, curr) => acc + curr.attempts, 0);
  const accuracy = history.length > 0 ? Math.round((history.length / (history.length + totalAttempts)) * 100) : 100;

  return (
    <div className="h-full w-full flex flex-col items-center justify-center p-6 text-center bg-pastel-purple">
      ...
      <button 
        onClick={onRestart}
        className="bg-kitty-dark hover:bg-black text-white px-12 py-5 rounded-3xl text-2xl md:text-3xl font-bold shadow-xl transition-all"
      >
        PLAY AGAIN 🔄
      </button>
    </div>
